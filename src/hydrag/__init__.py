...
# SurrealDB adapter (optional — requires hydrag-core[surrealdb])
try:
    from .surreal_adapter import SurrealDBAdapter

    _SURREAL_EXPORTS = ["SurrealDBAdapter"]
except ImportError:
    _SURREAL_EXPORTS = []

    def __getattr__(name: str) -> object:
        if name == "SurrealDBAdapter":
            raise ImportError(
                "SurrealDB adapter requires the 'surrealdb' package. "
                "Install it with: pip install hydrag-core[surrealdb]"
            )
        raise AttributeError(f"module 'hydrag' has no attribute {name!r}")
...
__all__ = [
    ...
    "compute_adaptive_n",
    "smart_truncate",
] + _TUNE_EXPORTS + _SURREAL_EXPORTS
