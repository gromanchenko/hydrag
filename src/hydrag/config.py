...
class HydRAGConfig:
    ...
    # SurrealDB adapter (optional — requires pip install hydrag-core[surrealdb])
    surrealdb_url: str = ""
    surrealdb_namespace: str = "hydrag"
    surrealdb_database: str = "default"
    surrealdb_timeout: int = 30
    surrealdb_graph_anchors: int = 3
    surrealdb_graph_max_neighbors: int = 50
    surrealdb_rrf_k: int = 60
    surrealdb_rrf_weights: str = "1.0,1.0"
    surrealdb_batch_size: int = 100
    surrealdb_max_in_flight: int = 256
    ...
    def __post_init__(self) -> None:
        ...
        if self.surrealdb_url:
            ...
            parsed = urlparse(self.surrealdb_url)
            if parsed.scheme not in ("http", "https", "ws", "wss"):
                raise ValueError(
                    f"surrealdb_url scheme must be http/https/ws/wss, got {parsed.scheme!r}"
                )
        # Parse comma-separated rrf_weights into tuple
        _parts = self.surrealdb_rrf_weights.split(",")
        if len(_parts) != 2:
            raise ValueError(
                f"surrealdb_rrf_weights must be two comma-separated floats, "
                f"got {self.surrealdb_rrf_weights!r}"
            )
        self._parsed_rrf_weights: tuple[float, float] = (
            float(_parts[0].strip()), float(_parts[1].strip())
        )
    ...
    def from_env(cls) -> "HydRAGConfig":
        ...
        cfg = cls(
            ...
            surrealdb_url=os.environ.get("HYDRAG_SURREALDB_URL", cls.surrealdb_url),
